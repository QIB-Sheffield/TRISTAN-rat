...
def main(study: str
    ...
    for substudy, day in list(itertools.product(fits.keys(), [1, 2])):
        try:
            ...
            plots.get_deltaR1_plots(fits, substudy, 'Liver', study, is_fitted=True, YLIM=(-1.5, 4.5))
            # For observed liver data only
            print(f"{substudy}, Liver: Saving average deltaR1 plot")
            plots.get_deltaR1_plots(fits, substudy, 'Liver', study, is_fitted=False, YLIM=(-1.5, 4.5))
            # For observed spleen data only
            print(f"{substudy}, Spleen: Saving average deltaR1 plot")
            plots.get_deltaR1_plots(fits, substudy, 'Spleen', study, is_fitted=False, YLIM=(-0.25, 1))
        except KeyError:
            continue
    ...
    all_parameters_cleaned.replace({'Fstrength':field_strengths}, inplace=True)
    ...
    all_parameters_cleaned['Time_period'] = all_parameters_cleaned.loc[:,'Substudy']
    ...
    # Sort dataframe
    all_parameters_cleaned.sort_values(['Substudy',
                                        'Site',
                                        'Fstrength'],
                                        ascending=[True,
                                                   True,
    ...
    site = all_parameters_cleaned.query('Substudy<=4 and Symbol in @params and Day==1')
    # across field strengths (at specific site G)
    fieldstrength = all_parameters_cleaned.query('Substudy<=4 and Site=="G" and Symbol in @params and Day==1')
    # across field strengths (at specific centre and subject)
    fieldstrength_pairwise = all_parameters_cleaned.query('Substudy==13 and Symbol in @params and Rat!=[4, 5, 6]')
    ...
    # substudy
    for biomarker in params:
        one_way_anovas.append(analyses.get_anova_oneway(substudy,
                                                        biomarker,
                                                        None,
                                                        None,
                                                        'Substudy'))
    # site
    for biomarker, constant in list(itertools.product(params,
                                                      constants['site'])):
        one_way_anovas.append(analyses.get_anova_oneway(site,
                                                        biomarker,
                                                        'Fstrength',
                                                        constant,
                                                        'Site'))
    # fieldstrength
    for biomarker in params:
        one_way_anovas.append(analyses.get_anova_oneway(fieldstrength,
                                                        biomarker,
                                                        'Site',
                                                        'G',
                                                        'Fstrength'))
    ...
    for biomarker in params:
        one_way_anovas.append(analyses.get_anova_oneway(fieldstrength_pairwise,
                                                        biomarker,
                                                        'Site',
                                                        'G',
                                                        'Fstrength'))
    ...
    retest_data = (data
                   .remove_insufficient_data(all_parameters_cleaned
                                             .query("Symbol in @params and Substudy<=4"),
                                             study))
    ...
    combined = retest_data.merge(rifampicin_data, on=['Substudy',
                                                      'Site',
                                                      'Symbol',
                                                      'Rat'], how='outer', indicator=True)
    ...
    benchmarks['repeatability'] = overall_saline['repeatability']['mean'].values
