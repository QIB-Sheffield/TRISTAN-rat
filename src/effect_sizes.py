...
def remove_data_errors(parameter_data: pd.DataFrame,
    ...
    non-convergence of the model fitting algorithm, i.e., the output of boundary 
    values instead of true values, e.g., gadoxetate extraction fraction, E=100%.
    Additionally removes cases where only one acquisition per subject is present
    (i.e., day 1 or day 2 data are missing).
    ...
    Returns:
        Cleaned DataFrame.
    """
    data_pivoted = pd.pivot_table(parameter_data, values = 'Value', columns=['Symbol'],
    ...
    missing_days_removed = fit_errors_removed[fit_errors_removed.groupby(['Site', 'Drug', 'Rat']).transform('count')>1].dropna()
...
def get_stats(cleaned_parameter_data: pd.DataFrame,
    ...
    Calculates summary statistics conveying the observed effects of administered 
    test compounds on the biomarkers of interest, i.e., between day 1 and day 2, 
    individual subject and average subject values per site are calculated for 
    gadoxetate percent change, effect size + 95% confidence intervals (CI95), 
    and paired T-test p-values.
    ...
    Returns:
        A tuple (rats, rats_avg), where rats is a DataFrame containing statistical
        summaries per rat, whereas rats_avg is a DataFrame containing statistical
        summaries for average rat values per drug.
    """
...
def save_effect_sizes(cleaned_parameter_data: pd.DataFrame,
    ...
    Args:
        ...
        study: Study name of interest (e.g., 'SixTestCompounds').
        """
    data_pivoted = pd.pivot_table(cleaned_parameter_data[cleaned_parameter_data['Symbol'].isin(params)], 
                                            values = 'Value',
                                            index=variables + ['Rat'],
                                            columns='Day',
                                            margins=True,
